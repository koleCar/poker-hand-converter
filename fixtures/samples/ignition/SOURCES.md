...
| cash__NLHE-USD-5-10-201511.concatenated.partial.txt | 2015-11 | **REAL BYTES, SELF-DAMAGED — DO NOT MAKE A PARSER ACCEPT THIS** | Truncated mid-hand: the file declares one `Bovada Hand #3178646516` header and then simply stops partway, with a summary block whose `Seat+N:` lines describe a hand whose action was never fully written. There is no recoverable reading — the missing chips are missing, not encoded elsewhere. A parser should **refuse** this hand and record a conversion failure, not guess. Keep it as a corpus fact about real Ignition export damage and as a robustness fixture for the refusal path; do not 'fix' a parser to swallow it. |
...
| tour__NLHE-HU-USD-STT - $20-$1 - 201509.partial.hand.txt | 2015-09 | **REAL BYTES, SELF-DAMAGED — DO NOT MAKE A PARSER ACCEPT THIS** | Two hands spliced together **mid-line, with no newline between them**, so the second hand's header is swallowed into the first hand's action line. Verbatim at line 10: `Dealer : RaisBovada Hand #3260003105: HOLDEM Tournament #13214704 TBL#1, No Limit- Level 1 (10/20) - 2015-09-09 19:07:11` — the action was `Dealer : Raises ...`, truncated after `Rais`, with hand #3260003105's header concatenated onto it. The raise amount is destroyed and unrecoverable. Note this also defeats naive hand-splitting: a splitter keying on `^(Bovada|Ignition) Hand #` at line start misses the second header entirely and treats both hands as one. A parser should **refuse** both hands. Keep as a robustness fixture; do not 'fix' a parser to accept it. |
