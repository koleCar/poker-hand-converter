...
| cash__NLHE-10max-0.25-0.50-201204.post.dead.txt | same | 2026-09-17 | REAL | `Player0: post dead 0.50` — dead-blind posting grammar (no "as a dead bet" qualifier, unlike WPN). **The printed amount is not the full post**: contributions sum to 15.36 but `Total pot 14.83 + Rake 0.78` = 15.61, short by exactly one small blind (0.25). The dead small blind is never written down. See `chico.md` §6. |
...
| cash__NLHE-10max-USD-0.01-0.02-201605.winner.no.show.txt | same | 2026-09-17 | REAL | Winner who does not show cards at showdown. **Also the reference case for the two-`Total pot`-line shape**: `Total pot 0.42 \| Rake 0.04` appears *before* `*** SUMMARY ***` and is the **side** pot; `Total pot 0.29 \| Rake 0.04` appears inside and is the **main** pot (won by the 0.07 all-in). Contributions 0.75 − rake 0.04 = 0.71 = 0.42 + 0.29. Proves the last line is **not** the grand total. |
...
| cash__PLO-10max-USD-0.05-0.10-201209.txt | same | 2026-09-17 | REAL | Pot-Limit Omaha (4-card hands); **inline `shows` action lines have no brackets and space-separated cards** (`Player2 shows Jc Jd`) while the later **SUMMARY line for the same event does have brackets** (`showed [Jc Jd]`) — a real, easy-to-miss inconsistency within one file; also demonstrates in-hand table chat (`Player7 said "raise pot , bet and collect ...so easy"`) and mid-hand seat join/leave events (`Player10 joins the table at seat #3`, `Player4 has left the table`) interleaved directly in the action stream with no special delimiter; also demonstrates **multiple `Total pot X \| Rake Y` lines** per hand. **Two warnings on this file.** (1) **The header lies about the variant**: all 17 hands are headed `Hold'em Pot Limit` but every one deals four hole cards — verified 17/17 both ways. Derive the variant from the dealt-card count, never the header token; upstream fpdb-3 evidently agrees, since it files this under `PLO-`. (2) Hand `#1073059444` is **self-inconsistent source data**: four `Total pot` lines with *differing* rakes (`6.91\|1.35`, `3.95\|1.10`, `10.30\|0.60`, `11.55\|0.60`), exactly one winner listed, contributions summing to 34.41 against printed pots summing to 32.71 — nothing reconciles. Treat that hand as corrupt and skip it; do not derive a four-pot grammar from it. |
